...
@section('path')
    <a href="{{route('admin_home')}}">Admin</a> › <a href='{{ route('admin_order_items_list') }}'>OrderItem</a> > Edit
@endsection
...
<div id="product-box" class="content-list">
    ...
    <div class="p-4">
        <form action="{{ route('admin_order_item_edit' , [$order_item->id]) }}" method="POST">
            @csrf 
            @method('PUT')
                    ...
                    <tr class="mb-2" style="border-top:0.1px solid rgba(255 255 255/20%);">
                        ...
                        <td class="td">
                            <select id="order_id" name="order_id" class="input-select mt-2 mb-2">
                                <option value="{{$order_item->id}}" selected> Order {{$order_item->id}} </option>
                    ...
                    <tr class="mb-2" style="border-top:0.1px solid rgba(255 255 255/20%);">
                        ...
                        <td class="td"> 
                            <select id="product_id" name="product_id" class="input-select mt-2 mb-2">
                                <option value="{{$order_item->product_id}}" selected> {{substr($order_item->product->name,0,30)}} </option>
